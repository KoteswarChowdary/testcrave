//
//  DetailBirthdayGiftCardTableViewController.m
//  WeCraveProject
//
...
@interface DetailBirthdayGiftCardTableViewController ()
{
     NSUserDefaults *birthdayCardDetailsRetrieve;
}

@end

@implementation DetailBirthdayGiftCardTableViewController
...
- (void)viewDidLoad
{
    ...
    birthdayCardDetailsRetrieve = [NSUserDefaults standardUserDefaults];
    
    NSUserDefaults *birthdayGiftCardDetailsToDB = [NSUserDefaults standardUserDefaults];
    //[giftCardDetailsToDB setObject:self.giftDesc forKey:@"Desc"];
    [birthdayGiftCardDetailsToDB setObject:[birthdayCardDetailsRetrieve objectForKey:@"price"] forKey:@"Price"];
    [birthdayGiftCardDetailsToDB setObject:[birthdayCardDetailsRetrieve objectForKey:@"pId"] forKey:@"GiftId"];
    
    [birthdayGiftCardDetailsToDB synchronize];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row == 0) {
            ...
            NSString *string = [NSString stringWithFormat:@"\t\t\t%@",[birthdayCardDetailsRetrieve valueForKey:@"GiftName"]];
        ...
        if (indexPath.row == 1)
        {
            ...
            photo.image = [UIImage imageWithData:[birthdayCardDetailsRetrieve valueForKey:@"GiftImage"]];
        ...
        if (indexPath.row == 2)        {
            cell.backgroundColor = [UIColor clearColor];
            NSString *string = [NSString stringWithFormat:@ "                   Cost: %@$",[birthdayCardDetailsRetrieve valueForKey:@"GiftPrice"]];
        ...
        if (indexPath.row == 3)
        {
            ...
            cell.backgroundColor = [UIColor clearColor];
            NSString *string = [NSString stringWithFormat:@ "                   Descrption /n%@",[birthdayCardDetailsRetrieve valueForKey:@"GiftDesc"]];
            NSLog(@"dsata in string  2 %@",string);
            // cell.textLabel.textAlignment = NSTextAlignmentCenter;
            cell.textLabel.font = [UIFont fontWithName:@"HelveticaNeue-CondensedBlack" size:16.0];
            cell.textLabel.numberOfLines = 0;
...
{
    [self performSegueWithIdentifier:@"Birthday" sender:self];
}
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    
    
    if ([[segue identifier] isEqualToString:@"Birthday"])
    {
